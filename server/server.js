...
app.get("/api/evaluationsemp/:matricule", async (req, res) => {
  ...
  try {
    const result = await client.query(
      `SELECT ee.id, ee.employer_id, ee.evaluation_date, ee.note, ee.status, ee.valide,
              c.id AS compagnie_id, c.date_debut, c.status AS compagnie_status
       FROM evaluation_employer ee
       JOIN employe e ON ee.employer_id::VARCHAR = e.matriculesalarie
       JOIN compagnie c ON ee.compagnie_id = c.id
       WHERE e.matriculesalarie = $1
...
app.get("/api/reponses/:evaluationId", async (req, res) => {
  const { evaluationId } = req.params;
  try {
    const result = await client.query(
      `SELECT r.id, r.evaluation_id, r.question_id, r.notechef, r.noterh,
              q.texte AS question_texte, q.rubrique, q.code
       FROM reponse r
       JOIN question q ON r.question_id = q.id
...
app.get("/api/compagnies", async (req, res) => {
  try {
    const result = await client.query(
      "SELECT id, date_debut, status FROM compagnie ORDER BY date_debut DESC"
    );
    res.json(result.rows);
  } catch (error) {
...
app.get("/api/evaluations/compagnie/:compagnieId", async (req, res) => {
  const { compagnieId } = req.params;
  try {
    const result = await client.query(
      `SELECT ee.id, ee.employer_id, ee.evaluation_date, ee.note, ee.status, ee.valide,
              e.prenom, e.nom, e.matriculesalarie,
              c.id AS compagnie_id, c.date_debut, c.status AS compagnie_status
       FROM evaluation_employer ee
       JOIN employe e ON ee.employer_id = e.id
       JOIN compagnie c ON ee.compagnie_id = c.id
       WHERE ee.compagnie_id = $1
       ORDER BY e.nom, e.prenom`,
      [compagnieId]
    );
    res.json(result.rows);
  } catch (error) {
    console.error("Error fetching evaluations by compagnie:", error);
    res.status(500).json({ error: "Internal server error" });
  }
});
