...
import { Funnel, Pencil, Save, Trash2, X } from "lucide-react";
...
const ADcheftable = () => {
  ...
  const handleDelete = async (matricule: number) => {
    if (window.confirm("Voulez-vous vraiment supprimer ce chef ?")) {
      try {
        const response = await fetch(
          `http://localhost:8080/api/chef/${matricule}`,
          {
            method: "DELETE",
          }
        );
        if (response.ok) {
          setChefs((prev) => prev.filter((c) => c.matricule !== matricule));
        } else {
                    ...
                    {editIndex === index ? (
                      ...
                      <>
                        ...
                        <button
                          ...
                          onClick={() => handleDelete(chef.matricule)}
                          title="Supprimer"
                        >
